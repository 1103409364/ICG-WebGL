...
window.onload = function () {
  ...
  gl.bufferData(gl.ARRAY_BUFFER, 8 * (maxPoints + boardVertexNumber), gl.STATIC_DRAW);
  ...
  gl.bufferData(gl.ARRAY_BUFFER, 16 * (maxPoints + boardVertexNumber), gl.STATIC_DRAW);
...
function addChess(vBuffer, cBuffer, x, y, width, height) {
  // 获取gl坐标
  const { x: xGl, y: yGl } = vertex2Gl(x, y, width, height);
  // 获取棋盘坐标
  const { x: xBoard, y: yBoard } = vertex2Board(xGl, yGl, width, height);
  // 获取棋盘状态坐标
  const { x: xState, y: yState } = vertex2State(xBoard, yBoard);
  // 该位置有棋子return
  if (boardState[yState][xState]) return;
  // 1为黑子,2为白子
  boardState[yState][xState] = colorFlag ? 1 : 2;
  const chessVertex = vertexChess(xBoard, yBoard);
  gl.bindBuffer(gl.ARRAY_BUFFER, vBuffer);
  ...
  const { x: xScreen, y: yScreen } = vertex2Screen(xBoard, yBoard, width, height);
  lastEl.style.display = "block";
  lastEl.style.left = `${xScreen}px`;
  lastEl.style.top = `${yScreen}px`;
  // 检查是否获胜
  checkFinished(xState, yState, colorFlag);
  colorFlag = !colorFlag;
}
  ...
  for (let i = 0; i < 4; i++) {
    ...
    const nextChessMap = {
      row: {
        ...
        increase:
          x + increaseDirection <= boardSegment && boardState[y][x + increaseDirection] === chessType,
      },
      col: {
        decrease: y - decreaseDirection >= 0 && boardState[y - decreaseDirection][x] === chessType,
        increase:
          y + increaseDirection <= boardSegment && boardState[y + increaseDirection][x] === chessType,
      },
      diagonalLeft: {
        decrease:
          x - decreaseDirection >= 0 &&
          y - decreaseDirection >= 0 &&
          boardState[y - decreaseDirection][x - decreaseDirection] === chessType,
        increase:
          x + increaseDirection <= boardSegment &&
          y + increaseDirection <= boardSegment &&
