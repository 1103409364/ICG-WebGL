...
function divideTriangle(A, B, C, count, points) {
  if (count === 0) {
    ...
    const midOfAB = vec2.lerp(vec2.create(), A, B, 0.5);
    const midOfAC = vec2.lerp(vec2.create(), A, C, 0.5);
    const midOfBC = vec2.lerp(vec2.create(), B, C, 0.5);
...
function createTexture2D(
  ...
  width,
  height,
  { needsGenerateMipmaps, magFilter, minFilter, wrapS, wrapT } = {},
) {
